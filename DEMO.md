...
(reserved, background, free). Idle cgroups are never limited (`--min-cores`, default 0.5). The Plan card shows the same
table. `--only` keeps the demo from touching your terminal, which is busy too.
